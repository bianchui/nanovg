...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	for (i = 0; i < 12; i++) {
		char file[128];
		snprintf(file, 128, mapResPath("../example/images/image%d.jpg"), i+1);
	...
	data->fontIcons = nvgCreateFont(vg, "icons", mapResPath("../example/entypo.ttf"));
	if (data->fontIcons == -1) {
		printf("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", mapResPath("../example/Roboto-Regular.ttf"));
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", mapResPath("../example/Roboto-Bold.ttf"));
	if (data->fontBold == -1) {
		printf("Could not add font bold.\n");
		return -1;
	}
	data->fontEmoji = nvgCreateFont(vg, "emoji", mapResPath("../example/NotoEmoji-Regular.ttf"));
	if (data->fontEmoji == -1) {
